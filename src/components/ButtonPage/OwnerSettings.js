...
export default OwnerSettings = ({buttonId, button, modalVisible, setModalVisible, friends}) => {

    const [title, setTitle] = useState(button.title);
    const [msg, setMsg] = useState(button.message);

    const [openButtons, setOpenButtons] = useState(false);
    const [valueButton, setValueButton] = useState(button.pattern);
    const [itemsButtons, setItemsButtons] = useState(data);

    const [openFriends, setOpenFriends] = useState(false);
    const [valueFrinds, setValueFrinds] = useState(button.friends.map((friend) => friend.id));
